...
import type { OrgSnapshot, PartInfo, WorkStatus, VersionInfo } from '../../shared/types'
import { SerialQueue } from '../lib/queue'

let SQL: SqlJsStatic | null = null

// Works in the CJS Electron-main build and under vitest's ESM transform.
...
async function loadSqlJs(): Promise<SqlJsStatic> {
  if (SQL) return SQL
  // sql.js ships a wasm binary next to its entrypoint; locate it at runtime.
  const entry = req.resolve('sql.js')
  const wasm = path.join(path.dirname(entry), 'sql-wasm.wasm')

...
import { VOCAB } from '../../../shared/constants'

export function TopBar(props: { onRefresh: () => void; onOpenMirror: () => void }) {
  const state = useClientState()
  const { appConfig: cfg, connection, org } = state

  const addr =
    cfg.mode === 'host'
      ? (state.hostAddress ?? 'starting…')
      : cfg.serverIp
  ...
  return (
    <header className="flex h-11 shrink-0 items-center gap-3 border-b border-zinc-800 bg-zinc-950/90 px-3">
      <div className="flex items-center gap-2">
        ...
        {cfg.mode === 'host' && (
          <span className="rounded border border-sky-900 bg-sky-950 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-300">
            Hosting
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5 text-xs text-zinc-400">
        ...
        <button
          className="rounded px-1.5 py-0.5 font-mono text-[12px] text-zinc-300 hover:bg-zinc-800"
          title={cfg.mode === 'host' ? 'This machine is hosting the org' : 'Server this app copies from'}
        >
          {addr}
        </button>

...
export function ConnectBanner(props: { onEditServer: () => void }) {
  ...
  const target = `${appConfig.serverIp}:${appConfig.port}`
  ...
  return (
    <div className="flex h-16 shrink-0 items-center gap-3 overflow-hidden bg-red-700 px-4 text-red-50">
      ...
      <div className="ml-auto flex items-center gap-1 text-sm text-red-200">
        ...
        <button
          onClick={props.onEditServer}
          className="rounded-md border border-red-400/60 px-2.5 py-1 text-xs font-semibold text-red-50 hover:bg-red-600"
        >
          Edit server…
        </button>
      </div>
